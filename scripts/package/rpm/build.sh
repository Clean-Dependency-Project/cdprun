...
yum install -y --allowerasing ca-certificates git tar gzip findutils coreutils rpm-build rpmdevtools golang sqlite bash >/dev/null
yum clean all >/dev/null
mkdir -p "${CDP_OUTPUT_DIR}"

exec "${CDP_BINARY_PATH}" --log-level error package rpm \
  --runtime "${CDP_RUNTIME}" \
  --version "${CDP_VERSION}" \
