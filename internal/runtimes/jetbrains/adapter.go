// Package jetbrains provides shared runtime adapter logic for JetBrains IDE products.
package jetbrains
...
// ProductOptions configures runtime-specific JetBrains metadata.
type ProductOptions struct {
	RuntimeName        string
	DefaultProductCode string
	DefaultReleaseType string
	DefaultReleasesURL string
...
// Adapter implements runtime.RuntimeProvider for JetBrains downloads.
type Adapter struct {
...
// NewAdapterWithConfig builds a shared JetBrains adapter with runtime-specific options.
func NewAdapterWithConfig(
	...
	if strings.TrimSpace(opts.DefaultReleaseType) == "" {
		opts.DefaultReleaseType = "release"
	}
	if strings.TrimSpace(opts.DefaultReleasesURL) == "" {
		opts.DefaultReleasesURL = "https://data.services.jetbrains.com/products/releases"
	}
	if strings.TrimSpace(opts.ArtifactPrefix) == "" {
		opts.ArtifactPrefix = "jetbrains"
	}
...
func (a *Adapter) GetName() string {
	return strings.TrimSpace(a.opts.RuntimeName)
}

// GetEndOfLifeProduct returns empty: JetBrains desktop IDE products are not tracked on endoflife.date.
...
func (a *Adapter) productCode() string {
	...
	return strings.TrimSpace(a.opts.DefaultProductCode)
}
...
func (a *Adapter) releaseType() string {
	...
	return strings.TrimSpace(a.opts.DefaultReleaseType)
}
...
func (a *Adapter) releasesBaseURL() string {
	...
	return strings.TrimRight(strings.TrimSpace(a.opts.DefaultReleasesURL), "/")
}
...
func (a *Adapter) metadataUserAgent() string {
	...
	return strings.TrimSpace(a.opts.DefaultUserAgent)
}
...
func (a *Adapter) GetLatestVersion(ctx context.Context, opts runtime.VersionOptions) (endoflife.VersionInfo, error) {
	...
	return a.releaseToVersionInfo(rel), nil
}
...
func (a *Adapter) CreateDownloadTasks(versionInfo endoflife.VersionInfo, platforms []platform.Platform, outputDir string) ([]runtime.DownloadTask, error) {
	...
	prefix := strings.TrimSpace(a.opts.ArtifactPrefix)
	if prefix == "" {
		prefix = "jetbrains"
	}
	...
	for _, plat := range platforms {
		...
		if ver == "" {
			return nil, fmt.Errorf("unable to determine %s version for %s", a.GetName(), plat.Classifier)
		}

		ext := extensionFromDownloadURL(artifact.Link)
		fileName := fmt.Sprintf("%s-%s-%s%s", prefix, ver, plat.Classifier, ext)
...
// LoadPolicy is a no-op; JetBrains versions come only from releases JSON.
...
// ParseChecksumLine parses the first checksum token from a checksum file line.
...
// SetExpectedSHA256 stores expected hash by platform classifier.
...
// ExpectedSHA256 returns expected hash by classifier.
